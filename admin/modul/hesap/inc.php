...
<div class="subheader py-2 py-lg-4 subheader-solid" id="kt_subheader">
	<div class="container-fluid d-flex align-items-center justify-content-between flex-wrap flex-sm-nowrap">
		...
		<div class="d-flex align-items-center flex-wrap mr-2">
			<!--begin::Page Title-->
			<h5 class="text-dark font-weight-bold mt-2 mb-2 mr-5">Hesap Ekle</h5>
			<!--end::Page Title-->
			<!--begin::Actions-->
...
<div class="d-flex flex-column-fluid">
	...
	<div class="container">
		<div class="card">
			<form action="javascript:;" method="post" id="modul-hesap" data-ajaxform="true">
			<input type="hidden" class="form-control" name="ickisim" value="ekle">
			<input type="hidden" class="form-control" name="ilkkisim" value="hesap">
			<input type="hidden" class="form-control" name="hesap_dil" value="<?=$config["ayar"]["ayarlar_sitedil"]?>">
				...
				<div class="row">
					...
					<div class="col-md-6">
						<div class="form-group">
							...
							<input type="text" class="form-control" name="hesap_permalink">
							<small>Bu kısım web sitemizden görünen url linki olacak. Boş bırakırsan otomatik oluşur.</small>
						</div>
					</div>
					<div class="col-md-6">
						<div class="form-group">
							<label>Hesap Açıklama</label>
							<input type="text" class="form-control" name="hesap_desc">
						</div>
					</div>
					<div class="col-md-6">
						<div class="form-group">
							<label>Hesap Anahtar Kelimeler</label>
							<input type="text" class="form-control" name="hesap_keyw">
						</div>
					</div>
					<div class="col-md-12">
						<?=resimalan("hesap_resim","Bilgisayardan Resim Seçin","")?>
					</div>
					<div class="col-md-12">
						<div class="form-group">
							<label>Açıklama</label>
							<textarea type="text" class="form-control summernote" name="hesap_aciklama" id="kt_summernote_1"></textarea>
						</div>
					</div>
...
<div class="subheader py-2 py-lg-4 subheader-solid" id="kt_subheader">
	<div class="container-fluid d-flex align-items-center justify-content-between flex-wrap flex-sm-nowrap">
		...
		<div class="d-flex align-items-center flex-wrap mr-2">
			<!--begin::Page Title-->
			<h5 class="text-dark font-weight-bold mt-2 mb-2 mr-5">Hesap Düzenle</h5>
			<!--end::Page Title-->
			<!--begin::Actions-->
			<div class="subheader-separator subheader-separator-ver mt-2 mb-2 mr-4 bg-gray-200"></div>
			<!--end::Actions-->
...
<div class="d-flex flex-column-fluid">
	...
	<div class="container">
		...
		<div class="tab-content mt-5" id="myTabContent">
			...
			$vbak=query("SELECT * FROM ".prefix."_hesap WHERE hesap_gid='".g("id")."' and hesap_dil='".$yaz["dil_id"]."'");
			$vsay=rows($vbak);
			$voku=row($vbak);
			...
			<div class="tab-pane fade <?php if(g("dil")==$yaz["dil_id"]){ echo "show active"; }elseif($no<1 and g("dil")==""){echo "show active";}?>" id="dilpage-<?=$no?>" role="tabpanel" aria-labelledby="dilpage-tab-<?=$no?>">
				...
				<div class="card">
					...
					<input type="hidden" class="form-control" name="ilkkisim" value="hesap">
					<input type="hidden" class="form-control" name="hesap_dil" value="<?=ss($yaz["dil_id"])?>">
					<input type="hidden" class="form-control" name="hesap_id" value="<?=ss($voku["hesap_id"])?>">
					<input type="hidden" class="form-control" name="hesap_gid" value="<?=ss(g("id"))?>">
					...
					<div class="card-body">
						<div class="row">
							<div class="col-md-12">
								<div class="form-group">
									<label>Adı</label>
									<input type="text" class="form-control" name="hesap_adi" value="<?=ss($voku[$ilkkisim."_adi"])=="" ? ss($vanaoku[$ilkkisim."_adi"]) : ss($voku[$ilkkisim."_adi"])?>" required>
								</div>
							</div>
							<div class="col-md-6">
								<div class="form-group">
									<label>Permalink</label>
									<input type="text" class="form-control" name="hesap_permalink" value="<?=ss($voku["hesap_permalink"])?>">
									<small>Bu kısım web sitemizden görünen url linki olacak. Boş bırakırsan otomatik oluşur.</small>
								</div>
							</div>
							<div class="col-md-6">
								<div class="form-group">
									<label>Hesap Açıklama</label>
									<input type="text" class="form-control" name="hesap_desc" value="<?=ss($voku["hesap_desc"])?>">
								</div>
							</div>
							<div class="col-md-6">
								<div class="form-group">
									<label>Hesap Anahtar Kelimeler</label>
									<input type="text" class="form-control" name="hesap_keyw" value="<?=ss($voku["hesap_keyw"])?>">
								</div>
							</div>
							<div class="col-md-12">
								<?=resimalan("hesap_resim","Bilgisayardan Resim Seçin",$voku[$ilkkisim."_resim"]=="" ? $vanaoku[$ilkkisim."_resim"] : $voku[$ilkkisim."_resim"])?>
							</div>
							<div class="col-md-12">
								<div class="form-group">
									<label>Açıklama</label>
									<textarea type="text" class="form-control summernote" name="hesap_aciklama" id="kt_summernote_1"><?=ss($voku["hesap_aciklama"])?></textarea>
								</div>
							</div>
...
<div class="subheader py-2 py-lg-4 subheader-solid" id="kt_subheader">
	<div class="container-fluid d-flex align-items-center justify-content-between flex-wrap flex-sm-nowrap">
		...
		<div class="d-flex align-items-center flex-wrap mr-2">
			<!--begin::Page Title-->
			<h5 class="text-dark font-weight-bold mt-2 mb-2 mr-5">Hesap Listele</h5>
			<!--end::Page Title-->
			<!--begin::Actions-->
		...
		<div class="card card-custom">
			<div class="card-header">
				<div class="card-title">
					...
					<h3 class="card-label">Hesap Listele</h3>
				</div>
			...
			<div class="card-body" id="yazdir">
				<?php
				$kurallar = "hesap_id=hesap_gid and";
				
				if(g("arama")!=""){
					$kurallar = $kurallar." (hesap_adi LIKE '%".g("arama")."%'
					) and";
				}
				...
				$sayfa = g("s") ? g("s") : 1;
				$ksayisi=rows(query("SELECT * FROM ".prefix."_hesap ".$kurallar));
				$limit=20;
				$ssayisi=ceil($ksayisi/$limit);
				$baslangic=($sayfa*$limit)-$limit;			  
			  
				$bak=query("SELECT * FROM ".prefix."_hesap 
				".$kurallar." ORDER BY hesap_sira DESC LIMIT $baslangic,$limit");
				$tableDizi = array();
				while($yaz=row($bak)){
					$tableDizi[] = $yaz;
				}
				...
				<!--begin: Datatable-->
					<table class="table">
						...
						<tbody>
							...
							<tr id="satir<?=$deger["hesap_id"]?>">
								<th scope="row"><?=$deger["hesap_id"]?></th>
								...
								<td><?=date("d.m.Y H:i",$deger["hesap_kayitzaman"])?></td>
								<td><a href="javascript:;" data-islem="modul-<?=$ilkkisim?>" data-deger="<?=$deger["hesap_id"]?>" data-ickisim="durum" id="durumbuton<?=$deger["hesap_id"]?>"><span class="label label-inline label-<?=$deger["hesap_durum"]==1 ? "success" : "warning"?>"><?=$deger["hesap_durum"]==1 ? "Aktif" : "Pasif"?></span></a></td>
								<td>
									<a href="<?=url.$deger[$ilkkisim."_permalink"]?>" class="label label-inline label-success" target="_blank">Gör</a>
									<a href="index.php?mo=<?=$ilkkisim?>_duzenle&id=<?=$deger["hesap_id"]?>" class="label label-inline label-info">Düzenle</a>
									<a href="javascript:;" data-islem="modul-<?=$ilkkisim?>" data-deger="<?=$deger["hesap_id"]?>" data-ickisim="sil" class="label label-inline label-danger">Sil</a>
								</td>
							</tr>
							<?php } ?>
						</tbody>
