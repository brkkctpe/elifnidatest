...
if($_POST){
	if($config["uye"]["uye_id"]>0){
		...
		if($_FILES["olcum_resim"]["tmp_name"]!=""){
			...
			$olcum_resim=ass(p("olcum_resimg"));		
		}
		
		$olcumKont = rows(query("SELECT * FROM ".prefix."_olcum WHERE olcum_uye='$olcum_uye' and olcum_id='$olcum_id' "));
		$telKont = rows(query("SELECT * FROM ".prefix."_uyeler WHERE uye_tel='$uye_tel' and uye_id!='$u_id' "));
		
		if($olcumKont<1){
			$json["tost"] = "warning";
			$json["mesaj"] = "Bu ölçüm size ait değil.";
			$json["uyari"] = '<div class="alert alert-warning">Bu ölçüm size ait değil.</div>';
		}elseif($olcum_tarih=="" or $olcum_agirlik==""){
			$json["tost"] = "warning";
			$json["mesaj"] = "Lütfen tüm alanları doldurun.";
			$json["uyari"] = '<div class="alert alert-warning">Lütfen tüm alanları doldurun.</div>';
		}else{
			$query="UPDATE ".prefix."_olcum SET
