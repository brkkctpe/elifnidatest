...
if($_POST){
	...
	elseif(($Kontrol->success) != "1" and $siteayar["ayarlar_mailrecaptcha"]==1){
		...
		if($_FILES["iletisim_cv"]["tmp_name"]!=""){
			$kaynak		 = $_FILES["iletisim_cv"]["tmp_name"];
			$resim 		 = $_FILES["iletisim_cv"]["name"];
			$rtipi		 = $_FILES["iletisim_cv"]["type"];
			$rboyut		 = $_FILES["iletisim_cv"]["size"];
			$hedef 		 = "../File";
			$iletisim_cv=dosyayukle($kaynak,$resim,$rtipi,$rboyut,$hedef);			
		}
		$iletisim_mesaj = $iletisim_mesaj.'<br> CV : <a href="'.siteurl."app/File/".$iletisim_cv.'">Yüklenen Dosyayı Gör</a>';
		...
		if($ekle){
								...
								<table class="content" align="center" cellpadding="0" cellspacing="0" border="0">
									...
									<tr>
										<td style="
											...
											">
											<h3>Yeni Bir İnsan Kaynakları Mesajı</h3>
										</td>
									</tr>
									<tr>
										<td style="background:#ffffff;padding:20px;">
											<h4>Ad Soyad : <?=$iletisim_adsoyad?></h4>
											<h4>Telefon : <?=$iletisim_telefon?></h4>
											<h4>Mail : <?=$iletisim_mail?></h4>
											<p><?=$iletisim_mesaj?></p>
										</td>
									</tr>
								</table>
			...
			$sonuc = mailgonder($alici,info("sitetitle"),pd("Yeni Insan Kaynaklari Formu"),$icerik,$dosya);
			$json['uyari'] = '<div class="alert alert-success" role="alert">Mesajınızı aldık en kısa sürede sizinle iletişim kuracağız.'.$output.'</div>';
			$json['sil'] = 1;
		}else{
